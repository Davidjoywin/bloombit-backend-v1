...
class AppointmentAvailability(models.Model):
    ...
    weekday = models.CharField(choices=time_available, max_length=20)
    weekend = models.CharField(choices=time_available, max_length=20)
...
class SpecialistProfessionalDetails(models.Model):
    ...
    medical_licence_number = models.CharField(max_length=30, blank=False, null=False)
...
class SpecialistVerificationDocuments(models.Model):
    ...
    medical_degree_certicate = models.ImageField(upload_to="./specialist/account", blank=False, null=False)
...
class RegisterSpecialist(models.Model):
    id = models.UUIDField(primary_key=True, auto_created=True, blank=True)
    ...
    def __str__(self):
        return f"Specialist: {self.personal_information.firstname} {self.personal_information.lastname}"
